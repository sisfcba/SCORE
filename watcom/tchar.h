...
inline size_t _ustrncnt( const wchar_t *__p, size_t __l ) {
    size_t __i;
    __i = _ustrlen(__p);
    return( (__i>__l) ? __l : __i );
}
inline wchar_t *_ustrspnp( const wchar_t *__p1, const wchar_t *__p2 ) {
    return( (*(__p1 += _ustrspn(__p1,__p2))!='\0') ? (wchar_t*)__p1 : NULL );
}
#else
