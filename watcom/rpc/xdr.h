...
typedef struct {
	...
	struct xdr_ops {
		...
		bool_t	(*x_getbytes)(void *, void *, u_int);/* get some bytes from " */
		bool_t	(*x_putbytes)(void *, void *, u_int);/* put some bytes to " */
		u_int	(*x_getpostn)(void *);/* returns bytes off from beginning */
		bool_t  (*x_setpostn)(void *, u_int);/* lets you reposition the stream */
		long *	(*x_inline)(void *, u_int);	/* buf quick ptr to buffered data */
		void	(*x_destroy)(void *);	/* free privates of this xdr_stream */
	} *x_ops;
	char *		x_public;	/* users' data */
	char *		x_private;	/* pointer to private data */
	char *		x_base;		/* private used for position info */
	long		x_handy;	/* extra private word */
} XDR;
...
#define XDRREC_READBYTES(xdrs, addr, len)		\
	(*(xdrs)->x_ops->x_getbytes)(xdrs, addr, len)
#define xdrrec_readbytes(xdrs, addr, len)		\
	(*(xdrs)->x_ops->x_getbytes)(xdrs, addr, len)
